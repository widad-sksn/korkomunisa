    ...
    <x-slot name="header">
        <div class="max-w-3xl mx-auto">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Perbarui portofolio kegiatan komisariat.
            </p>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 sm:rounded-2xl overflow-hidden">
                <div class="p-6 md:p-8">
                    <form action="{{ route('portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Ganti Gambar / Foto (Opsional)</label>
                            
                            @if($portfolio->image_path)
                                <div class="mb-4 p-4 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900/30 flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-900 dark:text-white">Foto Tersimpan</p>
                                            <a href="{{ asset('storage/' . $portfolio->image_path) }}" target="_blank" class="text-xs text-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:underline">Lihat foto saat ini &rarr;</a>
                                        </div>
                                    </div>
                            ...
                            <div x-data="{
                                ...
                                <div class="relative border-2 border-dashed rounded-xl p-2 flex flex-col items-center justify-center text-center transition-colors duration-200 overflow-hidden"
                                    ...
                                    <input type="file" name="image" id="image" x-ref="fileInput" @change="handleSelect" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*" :class="previewUrl ? 'hidden' : ''">
                                    ...
                                    <template x-if="!fileName">
                                        <div class="p-8 flex flex-col items-center pointer-events-none">
                                            ...
                                            <p class="text-base font-semibold text-gray-700 dark:text-gray-300 mb-1">Pilih Foto Baru atau Tarik ke sini</p>
                        ...
                        <div class="flex items-center gap-4 mt-4 pt-6 border-t border-gray-100 dark:border-gray-700">
                            <button type="submit" :disabled="isSubmitting" :class="isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:bg-gray-800 dark:hover:bg-gray-100'" class="px-6 py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-semibold rounded-lg focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 transition-all duration-200 shadow-sm flex items-center">
                                ...
                                <span x-text="isSubmitting ? 'Memproses...' : 'Perbarui Kegiatan'"></span>
                            </button>
